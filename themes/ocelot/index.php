...
$layout = get_theme_mod( 'craft_blog_blog_display_layout_options', 'masonry2-rsidebar' );

get_header(); ?>

<?php if( !empty( $mainslider ) && $mainslider == 'on' ){ ?>	

	<div class="ol-fullslider cS-hidden1">
		<?php
		...
		?>
			<div class="ol-slider-panel">
				...
				<div class="ol-img-holder">
					<?php
						the_post_thumbnail( 'ocelot-full-slider' );
					?>
				</div>
			</div>
		<?php } ?>
					...
					<?php
						if ( have_posts() ) :
							...
							the_posts_pagination( 
			            		array(
								    'prev_text' => esc_html__( 'Prev', 'ocelot' ),
								    'next_text' => esc_html__( 'Next', 'ocelot' ),
								)
				            );
